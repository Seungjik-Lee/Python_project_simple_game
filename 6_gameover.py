...
while running:
    ...
    character_x_pos += (character_to_x_LEFT + character_to_x_RIGHT) #-면 왼쪽 +면 오른쪽
...
#----------------------------------------왼쪽으로 튕겨나가는 작은 공                   
                    balls.append({
                        ...
                        "to_x" : -3, #공의 x축 이동방향,-3이면 왼쪽, +3이면 오른쪽
                        "to_y" : -6, #공의 y축 이동방향,
                        "init_spd_y" : ball_speed_y[ball_img_idx]}) #y 최초 속도
...
#----------------------------------------타이머 집어 넣기, 경과 시간 계산
    ...
    timer = game_font.render("Time : {}".format(int(total_time - elapsed_time)), True, (255, 255, 255)) #출력할 글자, True, 글자 색상
